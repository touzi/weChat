...
public class WeixinMsgController extends MsgController {
	...
	private static final String helpStr = "\t发送 help 可获得帮助，发送 news 可看新闻，发送 music 可听音乐，你还可以试试发送图片、语音、位置、收藏等信息，看会有什么 。公众号持续更新中，想要更多惊喜欢迎每天关注 ^_^";
...
//		ac.setEncodingAesKey(PropKit.get("encodingAesKey", "setting it in config file"));
	...
	protected void processInTextMsg(InTextMsg inTextMsg) {
		...
		String fromUserName = inTextMsg.getFromUserName().trim();
		ReqMsgLog reqMsgLog = getModel(ReqMsgLog.class);
		// 帮助提示
		if ("help".equalsIgnoreCase(msgContent)) {
			OutTextMsg outMsg = new OutTextMsg(inTextMsg);
			outMsg.setContent(helpStr);
			reqMsgLog.saveInTextMsg(inTextMsg, outMsg, pa);
			render(outMsg);
		}
		// 图文消息测试
		...
		//验证绑定
			...
			reqMsgLog.saveInTextMsg(inTextMsg, str, pa);
			renderOutTextMsg(str);
		}
		// 其它文本消息直接返回原值 + 帮助提示
		else {
			String str = "\t文本消息已成功接收，内容为： " + inTextMsg.getContent() + "\n\n" + helpStr;
			reqMsgLog.saveInTextMsg(inTextMsg, str, pa);
			renderOutTextMsg(str);
		}
	}
